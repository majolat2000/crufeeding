    ...
    <SafeAreaView className="flex-1 bg-[#F4F5F7]">
      <ScrollView className="flex-1" contentContainerStyle={{ paddingBottom: 24 }} showsVerticalScrollIndicator={false}>
        ...
        <View className="flex-row items-center justify-between px-5 pt-3 pb-4 bg-white border-b border-gray-100">
          ...
          <View className="bg-[#1A153B] px-3 py-1.5 rounded-full">
            <Text className="text-white text-[11px] font-bold">{session}</Text>
          </View>
        </View>
        ...
        <View className="px-3 mt-6">
          <View className="flex-row items-center justify-between px-2 mb-2">
            ...
            <Text className="text-xs text-gray-400">{RESTAURANTS.length} vendors</Text>
          </View>
          <FlatList
            data={RESTAURANTS}
            keyExtractor={(i) => i.id}
